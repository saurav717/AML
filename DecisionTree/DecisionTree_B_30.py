...
def run_decision_tree():
    ...
    X28 = data[B[27]:B[28]]
    X29 = data[B[28]:B[29]]
    X30_testing_data = data[B[29]:B[30]]


    training_data_list = [X1, X2, X3, X4, X5, X6, X7, X8, X9, X10, X11 , X12, X13, X14, X15, X16, X17, X18, X19, X20, X21, X22, X23, X24, X25, X26, X27, X28, X29, X30_testing_data]
    # training_set = np.roll(training_set, 1 , 0)
    # training_set = np.array(training_set).astype(float)

    sum_accuracy = 0
    for i in range (0,k_divisions):

        training_set = training_data_list[0].T
        for j in range (0,k_divisions-1):
            training_set = np.hstack((training_set, training_data_list[j+1].T))
        ...
        results = []
        test_set = training_data_list[k_divisions-1]
    ...
    print "final accuracy = ", final_accuracy/k_divisions
